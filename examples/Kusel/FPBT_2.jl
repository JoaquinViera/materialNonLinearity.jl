...
# arcLengthIncrem = vcat(ones(35) * 1e-5, ones(14) * 1e-6) # dips
arcLengthIncrem = vcat(ones(30) * 5e-5)
nLoadSteps = length(arcLengthIncrem)

controlDofs = [(n1 + 1) * 3] #
# controlDofs = collect((n1+1):(nnodes-1)) * 3 #
...
plot!(fig2, dVec_paper[1:10], pVec_paper[1:10])
...
# plot!(fig, kappaVec_paper[1:10], mVec_paper[1:10])
